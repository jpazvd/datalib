...
{title:Description}
{pstd}{cmd:_ctrycheck} checks the countries for which data is archived in the specified directory ({cmd:path}) by extracting unique country codes from the folder names. This utility is particularly useful for managing and verifying country-specific data stored in a structured repository.{p_end}
{pstd}The program identifies and lists unique country codes by evaluating the directory structure and filename patterns. It supports filtering based on {cmd:master} and {cmd:adaptation} file types.{p_end}
...
{synopthdr:Options}
{synoptline}
{synopt:{opt path(string)}}Specifies the directory path to check for ctry codes. This option is required.{p_end}
...
{p 6 16 2}Lists all unique country codes found in the {cmd:D:\datalib\BRA\} directory.{p_end}
{p 8 12}{stata "_ctrycheck , path(D:\datalib\BRA\)" :. _ctrycheck , path(D:\datalib\BRA\)}{p_end}

{p 6 16 2}Lists all unique country codes found in the {cmd:D:\datalib\BRA\BRA_2012_PNADC} directory.{p_end}
{p 8 12}{stata "_ctrycheck , path(D:\datalib\BRA\BRA_2012_PNADC)" :. _ctrycheck , path(D:\datalib\BRA\BRA_2012_PNADC")}{p_end}
...
{psee}
Suplementary functions: {helpb datalib} {helpb _dwl} {helpb _mkdir} {helpb _ctrycheck} {helpb _svycheck} {helpb _vcheck} {helpb _adaptcheck}
{p_end}