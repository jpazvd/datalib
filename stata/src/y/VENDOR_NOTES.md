...
Upstream splits the library across **two** directories — `stata/src/y/` for the
public `yaml*` commands and `stata/src/_/` for the `_yaml_*` internal helpers. We
...
|---|---|---|---|
| `_yaml_collapse.ado` | program | `stata/src/_/` | Wide-collapse helper; also defines the Mata functions `_yaml_binsearch`, `_yaml_mata_collapse`, `_yaml_safe_varname`, `_yaml_unique_varname` |
| `_yaml_fastread.ado` | program | `stata/src/_/` | Fast line-oriented reader path |
| `_yaml_mataread.ado` | program | `stata/src/_/` | Mata reader path; defines `_yaml_mata_parse` |
| `_yaml_tokenize_line.ado` | program | `stata/src/_/` | Line tokenizer |
...
| `upstream-README.md` | doc | README from upstream `stata/src/y/` |
