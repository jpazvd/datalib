...
*
* Measured on Z:/_staging_datalib-nut: 0.35 s per FOLDER, near-constant across subtrees
* of very different size (Spain 1,511 folders / 530 s; Afghanistan 119 / 42 s; Brazil
* 258 / 90 s). That is SMB round-trip latency per directory open, not overhead here:
* PowerShell's -Get-ChildItem -Recurse-, which enumerates in one process, took 538 s on
* the same subtree and found the same 1,510 folders and 2,548 files. There is no faster
* path from a single thread.
*
* So the whole staging tree -- 193 countries -- is a 6-to-8 hour walk, which is why
* -maxnodes()- defaults to 400 (about two and a half minutes) and why the cap is announced rather
* than silent. For the whole archive, use the scheduled catalogue under Z:/_inventory:
...
* 2,548 files, Spain with -sizes- is an hour on top of the walk. Without it, r(bytes)
