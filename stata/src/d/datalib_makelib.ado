...
*   pnad   dom, pes, both              (stata/src/i/ibge.ado; registry/examples)
*   pnadc  tri, anual; PANEL: idbas,idrs   (stata/src/i/ibge.ado)
...
program define _dl_mklib_vintage
    ...
    quietly {
        * The shared plan, not a seventh copy. This list used to omit
        * Data/Other, which every other creator in the package made --
        * a drift nothing could have caught while the plan was a literal.
        *
        * The vintage folder is created on its own line rather than as an
        * empty first element: the old list carried a LEADING slash on every
        * entry ("/Data") because the body concatenated `dir'`d' directly, and
        * the shared plan does not. Converting without moving the slash built
        * every folder one level up, which the DET and SMOKE suites caught.
        capture mkdir `"`dir'"'
        quietly _dtlb_folderplan
        foreach d in `r(subfolders)' {
            capture mkdir `"`dir'/`d'"'
        }
