...
{pstd}
{bf:Measured, not estimated.} On {cmd:Z:/_staging_datalib-nut} the walk costs about
{bf:0.35 seconds per folder}, and that figure is near-constant across subtrees of very
different size:

{p2colset 8 34 36 2}{...}
{p2col:{it:Spain}}1,511 folders, 2,548 files{space 4}530 s{p_end}
{p2col:{it:Brazil}}258 folders, 380 files{space 6}90 s{p_end}
{p2col:{it:Afghanistan}}119 folders, 300 files{space 6}42 s{p_end}
{p2colreset}{...}
...
walks the whole subtree in one process, took {bf:538 s} on {it:Spain} and found the same
1,510 folders and 2,548 files. {bf:There is no fast path from one thread.}

{pstd}
Two consequences worth knowing before you start something long:

{phang2}
1. The whole staging tree -- 193 countries -- is a {bf:6-to-8 hour} walk. That is why
...
under {cmd:Z:/_inventory}: it pays the same per-folder cost, but in parallel,
...
{phang2}{cmd:. datalib_index , root(Z:/_staging_datalib-nut) path("Spain/1975 Vital Statistics") dirs sizes clear}{p_end}

{pstd}What kinds of file does a country hold?{p_end}
{phang2}{cmd:. datalib_index , root(Z:/_staging_datalib-nut) path(Afghanistan) clear}{p_end}
