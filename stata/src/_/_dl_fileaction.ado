...
*! v1.12.0  13Aug2026   ported from datalib-unicef 0.9.26; POSIX quoting fixed
*******************************************************
...
*
* The buckets come from the real archive rather than from taste. Across the 55,703 files
* in Z:/_inventory the distribution is:
*
...
* caret, percent or backtick. Measured against the real archive that cost 406 of 55,703
...
program define _dl_fileaction, rclass
    ...
    * 17 extensionless files are text data (Spain's NACIA75 is a 34 MB fixed-width
    * extract), so the viewer is the correct guess, if a slow one for the larger ones.
    if ("`e'"=="") {
        ...
        * but the guard is PER PLATFORM, because the header's evidence is not.
        *
        * The header probed cmd, where a quoted path makes & | > ^ % inert. That
        * is true, and it is only true of cmd. POSIX sh expands $ and backticks
        * INSIDE double quotes, so `shell open "$(id).csv"` runs id. A double
        * quote is the only dangerous character on Windows; on POSIX it is one
        * of several.
        *
        * So POSIX gets single quotes, which suppress every expansion, and the
        * only character that can break out of them is a single quote itself.
        * Windows keeps double quotes, since cmd has no single-quote semantics.
        if ("`c(os)'"=="Windows") {
            if (strpos(`"`fp'"', char(34)) > 0) {
            ...
            * window title and opens a console instead of the file.
            ...
            if (strpos(`"`fp'"', char(39)) > 0) {
                return local kind "none"
                return local action ""
            }
            local opener = cond("`c(os)'"=="MacOSX", "open", "xdg-open")
            return local kind "open"
            return local action `"shell `opener' '`fp''"'
        }
    }
