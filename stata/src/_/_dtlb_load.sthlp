...
{title:Syntax}
{p 6 16 2}{cmd:_dlw} [varlist] {cmd:,} {cmd:country(string)} {cmd:year(string)} {cmd:survey(string)} [{cmd:MODule(string)} {cmd:filename(string)} {cmd:MASter} {cmd:adaptation} {cmd:LATest} {cmd:collection(string)} {cmd:harmonization(string)} {cmd:va(string)} {cmd:vm(string)} {cmd:WRK} {cmd:DEBUG} {cmd:data} {cmd:doc} {cmd:programs} {cmd:NOMerge} {cmd:cross(A B)} {cmd:NOWARNing} {cmd:clear}]{p_end}
...
{synopt:{opt cross(A B)}}Loads exactly two declared modules and pairs each record of one with each record of the other that shares its parent, instead of merging them. Use it where {opt module()} would merge: the two modules describe different characteristics of the same parent unit, and the question needs one row per pair. Refused with {opt filename()} (which skips module resolution, so there are no declared modules to pair), and refused when the two named modules are the same, are undeclared, or stand in an ancestor-descendant relationship. Sets {cmd:_dta[datalib_cross]}.{p_end}
{synopt:{opt NOWARNing}}Suppresses the join report. The report is printed by default and this turns it off, not the other way round: a join the user cannot see is a join they cannot check, and a silent merge that quietly returned the wrong shape is the failure this command exists to prevent.{p_end}
