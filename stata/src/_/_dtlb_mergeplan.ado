    ...
    forvalues a = 1/`nm' {
        ...
        forvalues b = `b'/`nm' {
            ...
            * non-contiguous ancestry: the deeper one carries the other's keys
            ...
            if (!`same' & !`anc') {
                * THIS is the refusal a caller actually reaches, because
                * naming both modules is the natural way to ask for both.
                * The message in _dtlb_load fires only when module() was left
                * empty, so until v1.10.1 the common path never mentioned
                * cross() -- and worse, told the reader the pairing was not a
                * thing you could have, which v1.10.0 had just made false.
                * Two refusal sites for one condition have to say the same
                * thing or the documentation is true of only one of them.
                ...
                if (`"`par_`a''"'==`"`par_`b''"' & `"`par_`a''"'!="") {
                    display as error `"{p}Both hang off {bf:`par_`a''}, so joining them is many-to-many: every row of one would repeat once per row of the other under the same parent. The default merge will not do that silently, because the result looks like a larger dataset rather than like an error.{p_end}"'
                    display as error `"{p}If the pair IS what you want, ask for it by name and the multiplication is measured and reported for you:{p_end}"'
                    display as error `"{p 8 8 2}{bf:cross(`nam_`a'' `nam_`b'')}{p_end}"'
                }
                display as error `"{p}Otherwise load one branch -- {bf:module(`nam_`a'')} -- or ask for a chain, for example {bf:module(`par_`a'' `nam_`a'')}.{p_end}"'
                exit 459
            }
        }
    }
