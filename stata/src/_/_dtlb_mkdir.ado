...
* _dtlb_mkdir.ado 
* Joao Pedro Azevedo
*! v1.11.0      <20260812>       JPAzevedo
*******************************************************

capture program drop _dtlb_mkdir
program define _dtlb_mkdir, rclass
    ...
    * The vintage folder plan, resolved ONCE for all four creation branches
    * below. It used to be six literal mkdir lines repeated four times in this
    * file, which is how Data/Other came to be created here and not by
    * datalib_makelib, and how none of the six ever created the three Doc/
    * subfolders Annex 1 specifies. See _dtlb_folderplan and
    * config/folderplan.yml.
    *
    * Resolved AFTER the line above, not before it. collection() without the
    * -adaptation- flag means an adaptation, and the inference that says so
    * happens here; asking for the plan any earlier read kind(master) for a
    * call that builds an adaptation folder. Under plan(default) the two are
    * the same tree, so nothing would have gone wrong today and everything
    * would have gone wrong the day a caller passed plan(ihsn2014), where an
    * adaptation takes Data/Harmonized and a bare Doc/. (Copilot, PR #92.)
    quietly _dtlb_folderplan, kind(`=cond("`adaptation'"!="", "adaptation", "master")')
    local _dtlb_plan `"`r(subfolders)'"'
        ...
        foreach ctry in `country' {
            ...
            foreach yr in `year' {
                ...
                *-----------------------------------------
                ...
                if ("`adaptation'" != "") {
                    _dtlb_adaptcheck, path("`path'/`ctry'/`ctry'_`yr'_`svy'")
                ...
                *-----------------------------------------
                cap: _dtlb_vcheck, path("`path'/`ctry'/`ctry'_`yr'_`svy'")
                local vcheck = _rc
                ...
                *-----------------------------------------
                if ("`adaptation'" == "") {
                    ...
                    if (`vcheck' != 0 & `Mcheck' == 0 & "`mkdir'" != "") {
                        ...
                        if (_rc == 0) {
                            foreach _s in `_dtlb_plan' {
                                cap: mkdir "`path'/`ctry'/`ctry'_`yr'_`svy'/`ctry'_`yr'_`svy'_`vmstr'_M/`_s'"
                            }
                ...
                *-----------------------------------------
                if ("`adaptation'" != "") {
                    ...
                    if (`vcheck' == 0 & `Mcheck' == 1 & `Acheck' == 0 & "`mkdir'" != "" & "`new'" == "new") {
                        ...
                        if (_rc == 0) {
                            foreach _s in `_dtlb_plan' {
                                cap: mkdir "`path'/`ctry'/`ctry'_`yr'_`svy'/`ctry'_`yr'_`svy'_`vmstr'_M_`vastr'_A_`clct'/`_s'"
                            }
...
cap: _dtlb_mkdir , path("D:\datalib\") country(ZZZ) survey(PNADC) year(2012)
 return list

 * Checks the vintage of data in the specified path for the 1981 PNAD survey in Brazil.
 _dtlb_mkdir , path("D:\datalib\") country(BRA) survey(PNAD) year(1981)
 return list

 * Checks the vintage of data in the specified path for the 2001 PNAD survey in Brazil.
 _dtlb_mkdir , path("D:\datalib\") country(BRA) survey(PNAD) year(2001)
 return list

 * Checks the vintage of data in the specified path for the 2001 PNAD survey in Brazil.
 cap: _dtlb_mkdir , path("D:\datalib\") country(BRA) survey(PNAD) year(2001) vm(01) va(01) adaptation 
 return list

 * Checks the vintage of data in the specified path for the 2012 PNADC survey in Brazil.
 _dtlb_mkdir , path("D:\datalib\") country(BRA) survey(PNADC) year(2012)
 return list

 * Checks the vintage of data in the specified path for the 2019 MICS survey in Bangladesh.
 _dtlb_mkdir , path("D:\datalib\") country(BGD) survey(MICS) year(2019)
 return list

 _dtlb_mkdir , path("D:\datalib\") country(BRA) survey(PNADC) year(2012) adaptation
 return list
