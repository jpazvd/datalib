...
*   4. stata/src/registry/catalogs.yaml          bundled offline fallback
...
program define _dtlb_catalogregistry, rclass
    ...
    if `resolved' == 0 {
        ...
        if `"`f_bundled'"' == "" {
            capture confirm file "stata/src/registry/catalogs.yaml"
            if _rc == 0 local f_bundled "stata/src/registry/catalogs.yaml"
        }
        if `"`f_bundled'"' != "" {
    ...
    if `resolved' == 0 {
        ...
        display as error `"  to add it: edit `f_user' (see stata/src/registry/catalogs.yaml for the schema)"'
        exit 198
    }
